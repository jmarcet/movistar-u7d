...
class Cache:
    ...
    def save_channels_data(xdata):
        ...
        clean_channels = {}
        for channel in (ch for ch in services if ch in EPG_CHANNELS):
...
class MulticastIPTV:
    ...
    async def __get_bin_epg(self):
        ...
        await asyncio.gather(
            *(self.get_epg_day(segments[key]["Address"], segments[key]["Port"], key) for key in segments)
        )
    ...
    def __get_channels(root):
        ...
        for svc in (s for s in root[0][0].iterfind("{urn:dvb:ipisdns:2006}SingleService") if len(s[0]) > 0):
            ...
            channel_list[channel_id] = {
                ...
                "logo_uri": svc[1].attrib["logoURI"] if "logoURI" in svc[1].attrib else "MAY_1/imSer/4146.jpg",
            }
    ...
    def __get_channels_keys(self):
        ...
        enabled_channels = set(_channels) & EPG_CHANNELS
    ...
    def __get_services(self):
        ...
        # Mpve "Portada HD" to last position, channel number 999
    ...
    async def get_epg_cloud(self):
        ...
        for program in cloud_recordings:
            ...
            timestamp = int(program["beginTime"] / 1000)
    ...
    async def get_epg_day(self, mcast_grp, mcast_port, source):
